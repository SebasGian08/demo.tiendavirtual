...
trait TProducto{
	...
	/* Mostrar Productos en la tienda TODOS LOS PRODUCTOS */
				...
				if(count($request) > 0){
					for ($c=0; $c < count($request) ; $c++) { 
						...
						if(count($arrImg) > 0){
							...
							$arrImg[0]['url_image'] = media().'/images/uploads/portada.jpg';
						}
	...
	public function getProductosCategoriaT(int $idcategoria, string $ruta){
		...
		if(!empty($request)){
			...
			$sql = "SELECT p.idproducto,
					...
					INNER JOIN categoria c
					ON p.categoriaid = c.idcategoria
					WHERE p.status != 0 AND p.status != 2 AND p.categoriaid = $this->intIdcategoria AND c.ruta = '{$this->strRuta}'";
					$request = $this->con->select_all($sql);
					if(count($request) > 0){
						for ($c=0; $c < count($request) ; $c++) { 
							$intIdProducto = $request[$c]['idproducto'];
	...
	public function getProductoT(int $idproducto,string $ruta){
		...
		$sql = "SELECT p.idproducto,
				...
				INNER JOIN categoria c
				ON p.categoriaid = c.idcategoria
				WHERE p.status != 0 AND p.status != 2 AND p.idproducto = '{$this->intIdProducto}' AND p.ruta = '{$this->strRuta}' ";
