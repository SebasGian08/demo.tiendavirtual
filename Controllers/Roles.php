<?php 
	...
	class Roles extends Controllers{
		...
		//Para mostrar los roles en el selector de Usuario
		public function getSelectRoles()
		{
			$htmlOptions = "";
			//extae todos los datos desde el modelo con selectRoles
			$arrData = $this->model->selectRoles();
			if(count($arrData) > 0 ){
				for ($i=0; $i < count($arrData); $i++) { 
					if($arrData[$i]['status'] == 1 ){
						//aca esta haciendo una validacion para que salga como html su id y su nombre rol
						// ejemplo: <option value="1">Activo</option>
		...
		//Metodo para listar en los campos para editar
		// int le quite por siacaso
